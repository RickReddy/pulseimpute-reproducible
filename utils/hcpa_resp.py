...
def load(mean=None, bounds=None, train=True, val=True, test=False, addmissing=False, path=os.path.join("data/pulseimpute_data/waveforms/hcpa_resp")):
    ...
    # Test
    if test:
        X_test = np.load(os.path.join(path, "hcpa_resp_test.npy")).astype(np.float32)
        if mean:
