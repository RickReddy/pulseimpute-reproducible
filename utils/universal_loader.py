...
# new file created by RG

def load(mean=None, bounds=None, train=True, val=True, test=False,
    addmissing=False, missingness_path=None,
    file_train=None, file_val=None, file_test=None):

    # Train
    if train:
        X_train = np.load(file_train).astype(np.float32)
        if mean:
    ...
    # Val
    if val:
        X_val = np.load(file_val).astype(np.float32)
        if mean:
    ...
    # Test
    if test:
        X_test = np.load(file_test).astype(np.float32)
        if mean:
            X_test -= np.mean(X_test,axis=1,keepdims=True) 
        if bounds:
            X_test /= np.amax(np.abs(X_test),axis=1,keepdims=True)*bounds
        X_test, Y_dict_test = modify(X_test, type="test", addmissing=addmissing, missingness_path=missingness_path)
    else:
        X_test = None
        Y_dict_test = None
...
def modify(X, type=None, addmissing=False, missingness_path=None):
    ...
    if addmissing:
        ...
        miss_tuples_path = missingness_path
